...
public class MainMenu {
		...
		menuList.addListener(new ClickListener(){
			...
			public void clicked(InputEvent event, float x, float y) {
                ...
                invEquipTable.add(getImage( "itemMeleeBig" ) ).padBottom(10);
                invEquipTable.add( equipWeapon ).padLeft(10).padBottom(10).expand();
                invEquipTable.add( new Label( statAtk.getText().toString(), skin, "big" ) ).row();
                invEquipTable.add(getImage( "itemArmorBig" ) ).padBottom(10);
                invEquipTable.add( equipArmor ).padLeft(10).padBottom(10).expand();
                invEquipTable.add( new Label( statDef.getText().toString(), skin, "big" ) ).row();
        ...
        private void updateItems()
        {
                ...
                for (int i = 0; i < itemsList.size(); i++) {
                        ...
                        invItemsTable.right().add( new Label( itemsList.get(i).name(), skin, "big" ) ).expand().fill().padLeft(20).padRight(50);
                        
                        if (itemsList.get(i).icon().equals("ranged") || itemsList.get(i).icon.equals("melee"))
                                if (itemsList.get(i).stat() > Integer.parseInt(statAtk.getText().toString())) {
                                        invItemsTable.add( compareItem( Integer.parseInt(statAtk.getText().toString()), itemsList.get(i).stat(), true ) );
                                } else
                                        invItemsTable.add( compareItem( Integer.parseInt(statAtk.getText().toString()), itemsList.get(i).stat(), false ) );
                        if (itemsList.get(i).icon().equals("armor"))
                                if (itemsList.get(i).stat() > Integer.parseInt(statDef.getText().toString())) {
                                        invItemsTable.add( compareItem( Integer.parseInt(statDef.getText().toString()), itemsList.get(i).stat(), true ) );
                                } else
                                        invItemsTable.add( compareItem( Integer.parseInt(statAtk.getText().toString()), itemsList.get(i).stat(), false ) );
                        if (itemsList.get(i).icon().equals("consumable"))
                                invItemsTable.add( new Label( "x " + Integer.toString(itemsList.get(i).stat()), skin, "big" ) );
        ...
        private Label compareItem( int stat, int item, boolean positive )
        {
                if (positive)
                        return ( new Label( ( "+" + Integer.toString( item - stat ) ), skin, "green" ) );
                return ( new Label( ( "-" + Integer.toString( stat - item ) ), skin, "red" ) );
        }
        ...
        private void getDemoItems()
        {
                ...
                itemsList.add( new ItemLabel("Me Arrrrmor", "armor", 2) );
